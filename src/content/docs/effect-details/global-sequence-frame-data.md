...
プラグインのこのインスタンスに固有のもの (UI 設定、テキスト文字列、パラメーターに保存されていないカスタムデータ) はすべてシーケンス データまたは新しい [Compute Cache For Multi-Frame Rendering](multi-frame-rendering-in-ae#compute-cache-for-multi-frame-rendering) に保存します。
