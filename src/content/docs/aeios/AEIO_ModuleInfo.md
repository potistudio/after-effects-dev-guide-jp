...
---
これは、AEIO がその基本プロパティを定義する構造です。

AEIO でサポートされるファイルタイプと拡張子を記述することに加えて、AEIO_ModuleFlags を使用して署名と動作も記述することに注意してください。私たちは旗が大好きです。

---

## AEIO_ModuleInfo メンバー

| Member | Purpose |
| --- | --- |
| `sig` | A long, uniquely identifying your plug-in.<br /><br />Many developers prefer to use a decidedly Mac-ish four character code here.<br /><br />Please [let us know](mailto:zlam@adobe.com) what sig you're using. |
...
| `num_aux_extensions` | The number of auxiliary extensions supported by your AEIO.<br /><br />Say, for example, that you're writing an AEIO to import information from a 3D program that saves scene information into a .123 file, and camera information into a .xyz file.<br /><br />The .xyz would be an auxiliary extension; it's not necessary to get the rest of the scene information, but it's associated with the .123 files. |
...
| `AEIO_MFlag_NO_TIME` | Time information isn't part of the file format.<br /><br />This would be the case with numbered stills, with individual frames imported based on the composition's time settings. | Input |
| `AEIO_MFlag_INTERACTIVE_GET` | A new input sequence necessitates user interaction.<br /><br />This would be the case for a non-file-based input module. | Input |
| `AEIO_MFlag_INTERACTIVE_PUT` | A new output sequence necessitates user interaction.<br /><br />This would be the case for a non-file-based output module. | Output |
...
| `AEIO_MFlag_INPUT_OPTIONS` | The AEIO has user options associated with each input sequence.<br /><br />!!! note<br />The options information must be flat (not referring to any data contained in external pointers or handles). | Input |
| `AEIO_MFlag_HSF_AWARE` | The AEIO will provide horizontal scaling factor (pixel aspect ratio) information for each new sequence.<br /><br />This prevents After Effects from guessing. | Input |
...
| `AEIO_MFlag_NO_UI` | After Effects should display no UI for this module<br /><br />(do not combine this flag with `AEIO_MFlag_HOST_DEPTH_DIALOG` or `AEIO_MFlag_HOST_FRAME_START_DIALOG`) | Input |
...
| `AEIO_MFlag_RESERVED4` | Special super-secret flag. Doesn't do anything...or does it?<br />(*No, it doesn't.*) | ??? |
